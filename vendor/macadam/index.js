...
// segfault-handler is an optional dev-only crash logger. It is a NAN addon
// (ABI-locked) that fails to load under Electron's Node ABI and is pruned from
// production builds, so guard it: load it when present (plain Node dev), skip
// it otherwise. Removing this guard breaks DeckLink under Electron. Mirrors the
// vendor/grandiose treatment of the same dependency.
try {
  const SegfaultHandler = require('segfault-handler');
  SegfaultHandler.registerHandler('crash.log');
} catch (_) { /* optional; unavailable under Electron / pruned in production */ }
